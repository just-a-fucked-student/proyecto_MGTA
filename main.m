...
%Pasar ETA a horas (esta en fraccion de dia)
...
%Guardar slots por si se necesitan despues
...
%% Cancelation and effect on main airlines with ground delay

chosenAirline = 'VLG';
delayThreshold = 180;

% Sacamos el codigo de aerolinea de los vuelos con ground delay
numGroundFlights = size(GroundDelay, 1);
airlineCodeGround = cell(numGroundFlights, 1);

for k = 1:numGroundFlights
    flightID = GroundDelay{k, 1};           % {} para extraer el string
    airlineCodeGround{k} = get_airline_code(char(flightID));
end

% Buscamos las 4 aerolineas principales dentro de GroundDelay
...
% Aplicamos cancelacion, substitution y compression
...
% Sacar codigo de aerolinea y delay de cada vuelo
...
% Agrupar por aerolinea: delay total y numero de vuelos
...
% Ordenar por delay total descendente y coger el top 10
[~, ordTotal] = sort(totalDelay, 'descend');
top = min(10, length(uniqueAL));
topIdx = ordTotal(1:top);

%Plot de aerolineas con mas delay medio
...
% Grafico Ground delay medio y Efecto de cancelar
figure;
