function compute_heatmaps(ARCID, ETA_hours, ETD_hours, Distances, Column_ECAC, Hstart, HNoReg, slots, llegadas)
    ...
    HM_CO2_Total = zeros(length(hfiles_test), length(radios_test));
    ...
    for i = 1:length(hfiles_test)
        for j = 1:length(radios_test)
            ...
            HM_CO2_Total(i, j) = total_CO2_air_loop + total_CO2_ground_loop;
            HM_Unrecoverable(i, j)  = total_unrecov;
    ...
    subplot(3,3,4);
    h4 = heatmap(str_radios, str_hfiles, HM_Unrecoverable);
    ...
    % Heatmap 6: Total CO2 Emissions
    subplot(3,3,6);
    h6 = heatmap(str_radios, str_hfiles, round(HM_CO2_Total));
    h6.Title = 'Total CO2 Emissions(kg)';
    h6.XLabel = 'Radius (km)';
    h6.YLabel = 'Hfile (Hora de publicación UTC)';
    h6.Colormap = hot;
    ...
    % 3D 6: CO2 Total
    subplot(3,3,6);
    surf(X, Y, HM_CO2_Total);
    title('Total CO2 Emissions(kg)');
    xlabel('Radius (km)'); ylabel('Hfile (h)'); zlabel('CO2 (kg)');
    colormap hot; colorbar;
end